...
const Alert = ({ alertObj }: { alertObj: { type: string, message: string } }) => {
  const { type, message } = alertObj;
  const [isShowing, setShowing] = useState(alertObj !== null ? true : false);
  let obj: { styles?: string, title?: string, message?: string, color?: string } = {}

  obj.color = 'black'
  obj.styles = `text-sm flex-col justify-between space-y-2 align-center absolute w-3/4 left-0 right-0 mx-auto bottom-[10%] rounded border px-3 py-3  ${type == "error" && "bg-red-100 border-red-400 text-red-700"
